...
export const generateOTP=async(username)=>{
    try{
        const text = `Thank you for reaching out to us and for choosing [Hospital Name] for your healthcare needs. 
                        We're delighted to confirm that we have received your bed reservation request`
        const subject = `Confirmation of Bed Reservation at [Hospital Name]`

        const {data} = await axios.post(`api/getMail`,{username, email, text, subject})
        return Promise.resolve({data})
    }
    catch(error){
        return Promise.reject({error})
